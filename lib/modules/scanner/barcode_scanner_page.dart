...
/// The calling page can then look up the product by SKU.
      ...
      Dialog(
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text("Enter SKU", style: Get.textTheme.titleLarge),
...
/// [onScanned] is called with the scanned barcode/QR string.
/// If the user cancels, onScanned is not called.
class BarcodeScannerHelper {
  /// Open the barcode scanner and handle the result.
  /// [context] is used for snackbars if needed.
  ...
  /// Look up a scanned/entered SKU and add it to cart.
  /// Shows appropriate snackbar feedback.
  static void addSkuToCart(String sku) {
  ...
  /// Look up a scanned/entered SKU and navigate to product edit.
  static void findProductBySku(String sku) {
    final productsController = Get.find<ProductsController>();

    final product = productsController.findBySku(sku);
    if (product != null) {
      // Switch to All category and search for the product
